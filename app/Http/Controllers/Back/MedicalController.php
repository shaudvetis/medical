...
use App\Models\{
    ...
    Medical\Planbag,
    Medical\SoPosoperplan,
    Medical\Sheduleoperblock
};
use DB;
use Auth;
use DateTime;
   ...
   foreach ($user as $users){
    ...
    $itog=DB::transaction(function() use ($request, $id, $soposNib) {
     ...
     $soposID=$soposNib->nib;
      ...
      $user=$soposNib;
         if ($request->bag_id != $request->oldbag_id) {
...
foreach ($m2 as $item) {
  ...
  $pal = Planbag::where('data',$item)->where('bag_id', $request->bag_id)->where('nib1',NULL)->where('bron', 0)->first();
...
// $post = So_po::find($id)->update(['posdiacode' => $request->posdiacode, 'posdiacodetext' => $request->posdiacodetext, 'p_oper' => $request->p_oper, 'user_id' => $request->user_id, 'd_operplan' => $request->d_operplan, 'd_posplan' => $request->d_posplan, 'd_outplan' => $request->d_outplan, 'palata_id' => $request->palata_id, 'p_operblock_id' => $request->p_operblock_id]);
...

     {
       ...
       $current1=Carbon::now()->format('Y-m-d');
       $dates1 = Carbon::now()->addDay(60);

         $current=Carbon::parse($request->post)->format('Y-m-d');
         $dates = Carbon::parse($request->out)->format('Y-m-d');
     ...
     $test = DB::select("select data, palat_id, id, m1.pac_fio,bag_name,color, bag_id, palat_name,nib1,bron, pac_sex1,pac_fio1 from planbags
      LEFT JOIN (select nib, nibcard, pac_fio from so_pos) as m1 on planbags.nib1=m1.nib
      where planbags.data BETWEEN  CAST('$current' as date) AND CAST('$dates' as date)  ");

     $rt=[];

     foreach ($test as $key => $value) {

         $rt[$value->palat_name.'-'.$value->bag_name]['nib'][]=$value->pac_fio;
         $rt[$value->palat_name.'-'.$value->bag_name]['bag_id'][]=$value->bag_id;
         $rt[$value->palat_name.'-'.$value->bag_name]['pac_sex'][]=$value->pac_sex1;
         $rt[$value->palat_name.'-'.$value->bag_name]['bron'][]=$value->bron;
         $rt[$value->palat_name.'-'.$value->bag_name]['color'][]=$value->color;
         $rt[$value->palat_name.'-'.$value->bag_name]['datas'][]=$value->data;
     }
...
//            exit();
       ...
       if ($request->ajax()) {
            if ($request->hook=='gr')
             ...
             return response()->json([
                 'table' => view("back.filter.brick-grpacient", ['m2' => $m2, 'rt' => $rt, 'cont' => $cont])->render(),
             ]);
       }
      ...
      return view('back.filter.brick-grpacient', compact('palat','m2','bag','rt'));
    }
  ...
  /**
    ...
    public function addsurgery (Request $request, $id)
    {
        ...
        $soDiagnoz = $this->repository->soDiagnoz($payout_id);

        //Запрос к справочнику инструментов + инструментам для пациента ранее записаных и выводим по нибу

        $all = $this->repository->soDeviceNib($id);

        //Получаем текущую дату для просчета недели
        $r = Carbon::now();

        //Если пришел запрос по аджаксу
        ...
        $opersb = Sheduleoperblock::select('worktime_start')->where('p_operblock_id', 1)->toBase()->get();

//dd($opersb);
        while ($st < $et) {
            $t9 = strtotime("+30 minute", $st);
            $tr = date("H:i", $st);
            $st = $t9;
//            Sheduleoperblock::create([
//               'worktime_start' => $tr,
//                'p_operblock_id' => 4,
//            ]);
            $times[] = $tr;
...
//        exit();
        ...
        $operblockoper = DB::select("SELECT id as idoperblock, opername,npp,startwork,endwork, m5.d_operplan, m1.pac_fio, m2.u2name,
       m3.fullname, m4.color,m5.nib, m5.timeoper, m5.p_operblock_id, m5.event_start, m5.ref from p_operblocks

           LEFT JOIN (select id as idsooper, nib,
        p_oper,p_payout_id,d_operplan,user_id,p_operblock_id,timeoper, event_start,ref from so_posopers  where  so_posopers.d_operplan between '$da' and '$da1'  order by p_operblock_id asc, so_posopers.d_operplan asc) as m5 ON p_operblocks.id=m5.p_operblock_id
         ...
         LEFT JOIN (select id as idpayout, color from  p_payouts ) as m4 ON m5.p_payout_id=m4.idpayout
          ...
          order by idoperblock asc, m5.d_operplan asc
         ");
//
//
        ...
        foreach ($period as $d34) {
            $m56[] = $d34->format('Y-m-d');
        }

        foreach ($period as $date) {
            foreach ($times as $key22 => $t) {

                $m32[1][$date->format('Y-m-d')][$t] = '';
...
$rts = [];
        ...
        foreach ($period as $key => $date) {
            foreach ($operblockoper as $key22 => $val) {
                ...
                if ($val->d_operplan == $date->format('Y-m-d') and  $val->idoperblock == $val->p_operblock_id )  {

                    for($i=1; $i<=$val->ref; $i++) {
                        $str = $val->pac_fio.$val->u2name.$val->fullname;
                        $n = 15;
                        $arr = str_split($str, $n);
                        //dd($arr);
                        $d = count($arr)-1;

                        $rts[$val->idoperblock][$date->format('Y-m-d')][] = $val->ref.'#'.$val->pac_fio.$val->u2name.$val->fullname.$val->color;
                        }
...
//       exit();
 if ($request->ajax()) {
     ...
     return view('back.filter.refsurgery', compact('opersb','operblockoper','item','times', 'soDiagnoz', 'm56','all','rts','m3', 'da', 'da1','mkb_group'));
