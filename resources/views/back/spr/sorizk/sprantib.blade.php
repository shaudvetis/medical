...
<ul class="nav nav-tabs">
  ...
  <li class="nav-item">
    @php $an=3; @endphp
    <a class="nav-link" href="{{route('sprrizki.edit', $an)}}">Тромбопрофілактика (л/з)</a>
  </li>
  <li class="nav-item">
    @php $tr=5; @endphp
    <a class="nav-link active" href="{{route('sprrizki.edit', $tr)}}">Антібіотікоровілактика (л/з)</a>
  </li>
  <li class="nav-item">
     @php $dr=6; @endphp
    <a class="nav-link" href="{{route('sprrizki.edit', $dr)}}">Інші (л/з)</a>
  </li>
</ul>
...
<button type="button" class="btn rounded-pill ex-button-0" data-bs-toggle="modal" data-bs-target="#exampleModal" > <i class="bi bi-plus"></i> Додати запис</button>
 <div class="card card-body mt-3">
  <h4 class="mt-1">Антібіотікоровілактика (л/з)</h4>
...
<table class="table table-bordered mt-2"  style="width: 90%">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class=" otstup " @if(!empty($item->color)) style="color:{{$item->color}};font-weight: bold;"  @endif>{!!$item->nametromb2!!} {!!$item->ntitle!!}  </td>
